...
class SubscriptionChecker:
    ...
    async def check_all_users(self):
        ...
        for sub in all_subs:
            ...
            if last_sub_detail and last_sub_detail.plan_id != 1:
                remaining_time = last_sub_detail.expired_at - now
    ...
    async def send_renew_message(self, sub, sub_detail, remaining_time):
        ...
        try:
            ...
            kb.button(
                text="✅ Check Payment",
                callback_data=f"check_payment:{sub.user_id}:{order_id}"
            )
            kb.adjust(1, 2)
    ...
    async def handle_check_payment(self, callback: types.CallbackQuery):
        try:
        ...
        if payment.status == "completed":
            ...
            await callback.answer("❌ Payment not completed yet. Please try again later.", show_alert=True)
